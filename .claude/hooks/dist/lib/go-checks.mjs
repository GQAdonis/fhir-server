...
export function goTarget(toolInput, projectRoot) {
    const target = targetPath(toolInput);
    if (target === undefined || !target.endsWith(".go"))
        return undefined;
 ...
 */
export function hasLicenseHeader(source) {
    const comments = [];
    for (const line of source.split(/\r?\n/, HEADER_WINDOW_LINES)) {
        ...
        else if (trimmed.startsWith("//go:build"))
            continue;
    }
    const block = comments.join("\n");
    return /Copyright \(c\) \d{4}, WSO2 LLC/.test(block) && /Apache License/.test(block) && /Version 2\.0/.test(block);
}
export function licenseFeedback(file, displayPath) {
    if (hasLicenseHeader(readFileSync(file, "utf8")))
        return undefined;
    return [
...
export function runGofmt(file, env = process.env) {
    ...
    if (r.status !== 0)
        return { kind: "error", detail: r.stderr.trim().split("\n")[0] ?? `exit ${r.status}` };
