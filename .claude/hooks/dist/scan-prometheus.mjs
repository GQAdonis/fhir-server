...
const { accepted: allowlist, rejected: rejectedEntries } = validateAllowlist(parseAllowlist(existsSync(allowlistFile) ? readFileSync(allowlistFile, "utf8") : undefined));
...
for (const file of files.sort()) {
    if (path.resolve(file) === path.resolve(allowlistFile))
        continue;
