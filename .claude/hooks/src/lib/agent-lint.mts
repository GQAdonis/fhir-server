...
export function parseAgent(source: string): ParsedAgent {
  ...
  if (match[1]!.trimStart().startsWith("{")) {
    ...
    if (obj === null || typeof obj !== "object" || Array.isArray(obj)) throw new Error("JSON frontmatter must be an object");
    for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
      if (typeof value === "string") frontmatter[key] = value;
      else if (Array.isArray(value) && value.every((v) => typeof v === "string")) frontmatter[key] = value as string[];
      // Nested native settings (e.g. OpenCode `permission`) are kept as JSON text.
      else frontmatter[key] = JSON.stringify(value);
    }
...
export function lintAll(ctx: LintContext): { agents: number; problems: string[] } {
  ...
  if (files.length === 0) problems.push("no .claude/agents/*.md files found");
