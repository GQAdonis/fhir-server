...
export function rosterIds(repoRoot: string): string[] | undefined {
  const f = path.join(repoRoot, ".agent-team", "team.json");
  if (!existsSync(f)) return undefined;
  const manifest = JSON.parse(readFileSync(f, "utf8")) as { roles: Role[] };
...
export function parseFlatToml(source: string): Record<string, string> {
  ...
  for (const raw of source.replace(/\r\n/g, "\n").split("\n")) {
    ...
    if (m === null) throw new Error(`unsupported TOML line: ${line.slice(0, 80)}`);
    const value = m[2]!;
    // Basic strings are JSON-compatible for the escapes the exporter emits.
    out[m[1]!] = value.startsWith('"') ? (JSON.parse(value) as string) : value;
  }
  return out;
}

function generatedSections(where: string, text: string): string[] {
  const problems: string[] = [];
  for (const heading of REQUIRED_SECTIONS) {
    if (!new RegExp(`^${heading}\\b`, "m").test(text)) problems.push(`${where}: missing section "${heading}"`);
  }
  if (!text.includes(PHI_HEADING)) problems.push(`${where}: missing "${PHI_HEADING}" block`);
  if (!text.includes(CARD_HEADING)) problems.push(`${where}: missing "${CARD_HEADING}"`);
  return problems;
}

interface HarnessSpec {
  readonly dir: string;
  /** Role ids present in the directory. */
  list(dir: string): string[];
  file(id: string): string;
  /** Problems for one role file's structure and generated sections. */
  check(where: string, source: string, id: string): string[];
}

const mdIds = (dir: string): string[] => readdirSync(dir).filter((f) => /^[a-z0-9][a-z0-9-]*\.md$/.test(f)).map((f) => f.slice(0, -3));

function mdAgent(requireName: boolean, extra?: (fm: Record<string, string | string[]>) => string[]) {
  return (where: string, source: string, id: string): string[] => {
    try {
      const { frontmatter: fm, body } = parseAgent(source);
      const problems: string[] = [];
      if (typeof fm["description"] !== "string" || fm["description"] === "") problems.push(`${where}: missing description`);
      if (requireName && fm["name"] !== id) problems.push(`${where}: name "${String(fm["name"])}" does not match ${id}`);
      if (extra) problems.push(...extra(fm).map((p) => `${where}: ${p}`));
  ...
  codex: {
    ...
    check: (where, source, id) => {
      try {
        ...
        const problems: string[] = [];
        for (const k of ["name", "description", "developer_instructions"]) if (!t[k]) problems.push(`${where}: missing "${k}"`);
        for (const k of Object.keys(t)) if (!CODEX_KEYS.has(k)) problems.push(`${where}: key "${k}" is not allowed in a generated Codex agent`);
        if (t["sandbox_mode"] !== undefined && !CODEX_SANDBOX.has(t["sandbox_mode"])) problems.push(`${where}: sandbox_mode "${t["sandbox_mode"]}" is not allowed`);
        if (GATE_ROLES.has(id) && t["sandbox_mode"] !== "read-only") problems.push(`${where}: gate role must have sandbox_mode = "read-only"`);
        if (t["name"] !== undefined && t["name"] !== id) problems.push(`${where}: name "${t["name"]}" does not match ${id}`);
...
export function lintHarnesses(repoRoot: string, roster: readonly string[]): string[] {
  const problems: string[] = [];
  for (const [name, spec] of Object.entries(HARNESSES)) {
    ...
    for (const id of roster) if (!present.includes(id)) problems.push(`${name}: ${spec.dir}/${spec.file(id)} is missing`);
    for (const id of present) if (!roster.includes(id)) problems.push(`${name}: ${spec.dir}/${spec.file(id)} is not a roster role`);
  ...
  for (const id of roster) if (!registered.includes(`[agents.${id}]`)) problems.push(`codex: .codex/config.toml does not register ${id} (Codex does not auto-discover agent files)`);
  const claude = path.join(repoRoot, ".claude", "agents");
  if (existsSync(claude)) for (const id of mdIds(claude)) if (!roster.includes(id)) problems.push(`claude: .claude/agents/${id}.md is not a roster role`);
  for (const id of roster) {
    const f = path.join(claude, `${id}.md`);
    if (!existsSync(f)) problems.push(`claude: .claude/agents/${id}.md is missing`);
    else problems.push(...generatedSections(`claude: .claude/agents/${id}.md`, readFileSync(f, "utf8")));
  }
  return problems;
}

/** Paths under .minimax/ that git would commit: only the marker, roster agent.md files and mirrored skills are allowed. */
export function minimaxCommittableProblems(repoRoot: string, roster: readonly string[]): string[] {
  ...
  if (r.status !== 0 || r.error !== undefined) return [`minimax: cannot list .minimax files with git (${(r.stderr || String(r.error ?? "")).trim().slice(0, 200)}); refusing to assume it is clean`];
...
export function driftProblems(repoRoot: string): string[] {
